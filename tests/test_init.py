...
def bootstrapped():
    ...
    with patch("openoutreach.core.management.commands.init.ensure_database"), \
            patch("openoutreach.core.management.commands.init.ensure_onboarded"), \
            patch("openoutreach.core.management.commands.init.validate_operator"), \
            patch("openoutreach.core.logging.print_banner"):
        yield
...
class TestTheFileFlags:
    ...
    def test_a_file_supplies_the_product_description(self, tmp_path, monkeypatch, bootstrapped):
        monkeypatch.delenv("OPENOUTREACH_PRODUCT_DESCRIPTION", raising=False)
        doc = tmp_path / "product.md"
        doc.write_text("# A self-hosted CI dashboard\n\nFor small dev teams.\n")

        from openoutreach.core.management.commands.init import _seed_environment
        _seed_environment({"product_docs": str(doc), "target": None, "name": None})

        import os
        assert "self-hosted CI dashboard" in os.environ["OPENOUTREACH_PRODUCT_DESCRIPTION"]
    ...
    def test_an_explicit_export_beats_the_file(self, tmp_path, monkeypatch, bootstrapped):
        ...
        monkeypatch.setenv("OPENOUTREACH_PRODUCT_DESCRIPTION", "from the environment")
        doc = tmp_path / "product.md"
        ...
        import os
        assert os.environ["OPENOUTREACH_PRODUCT_DESCRIPTION"] == "from the environment"
