"""Create the pipeline and the campaign, print what was created, and stop.
    ...
    openoutreach init                                  # wizard on a TTY, environment otherwise
    openoutreach init --product-docs p.md --target t.md # the two long fields, from files
    openoutreach init --json                           # the same campaign, for a program
...
class Command(OpenOutreachCommand):
    ...
    def _report(self, described: dict, options) -> None:
        ...
        self.stdout.write("")
        self.stdout.write("Next: openoutreach find 1")
...
def _seed_environment(options) -> None:
    ...
    """
    from openoutreach.core.onboarding import ENV_PREFIX
