...
2. **Balance-driven selection** — `n_negatives > n_positives` → exploit (highest P); else → explore (highest BALD). Anchors count as positives here, but the balance decides nothing while any of them stand — the cold phase is pinned to exploit (above) and the budget keeps the counts level regardless; both run against a real posterior from the first pass. If `acquisition_scores` still returns None the campaign is *unanchored* (LLM outage, no ICP text) — the degraded path, where selection falls back to `creation_date` order because nothing can rank.
...
- **Balancing is skipped while any of them stand.** `_balance` caps the majority at 2× the
  ...
  do anyway, since the anchor budget holds the classes level by construction. Their pull is local
  to their own neighbourhood (RBF kernel), which is the shape wanted. Balancing takes over when
  the last anchor retires, which is exactly when both classes are real and can diverge again.
- **One budget governs both their growth and their retirement**, and it is a subtraction:
  `BayesianQualifier.anchor_budget = max(0, n_neg - n_real_pos)` — invented positives fill only
  the gap ground truth has not closed. A rejection widens it and `pools._rebalance_anchors` tops
  the set up (`ensure_anchors` asks for the shortfall and shows the model what it has already
  written, so a top-up widens the ideal region instead of restating it; it fires only once the
  gap is a full `ANCHOR_COUNT` wide, which is what keeps this at one LLM call per `ANCHOR_COUNT`
  rejections rather than one per rejection). An acceptance narrows it and `_retire_anchors` drops
  one, newest first — the late anchors are top-up padding, the first `ANCHOR_COUNT` are the
  campaign's original statement of its ICP and go last. Without the budget a cold phase left
  alone slides to 3-against-hundreds and the posterior flattens toward "no" everywhere, which is
  the blindness anchors exist to remove.
- **They retire one per real acceptance, not all at once.** Dropping the whole set on the first
  positive could not converge: it took a positive class of dozens to one against hundreds of
  rejections in a single step, re-creating that same flat posterior one lead into the campaign's
  real evidence. Under the budget the handover is one-for-one and the last anchor goes when real
  positives *reach the rejection count* — the point where removing the padding still leaves the
  positive class the larger of the two, so nothing about the fit lurches when it happens. In
  practice this is what keeps a campaign in lead-search mode long enough to accumulate a real
  positive class instead of pivoting off a single acceptance.
- **`is_cold` (any anchor still standing) — not `has_real_positive`, not "is it fitted?" — is the
  engine's phase test.** Retirement is written through to `Campaign.anchor_profiles` /
